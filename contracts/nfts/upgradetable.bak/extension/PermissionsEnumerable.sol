...
library PermissionsEnumerableStorage {
  ...
  bytes32 public constant PERMISSIONS_ENUMERABLE_STORAGE_POSITION =
    keccak256(
      abi.encode(uint256(keccak256("permissions.enumerable.storage")) - 1)
...
contract PermissionsEnumerable is IPermissionsEnumerable, Permissions {
  ...
  function getRoleMember(
    ...
    uint256 currentIndex = _permissionsEnumerableStorage()
      .roleMembers[role]
      .index;
    uint256 check;
    ...
    for (uint256 i = 0; i < currentIndex; i += 1) {
      if (
        _permissionsEnumerableStorage().roleMembers[role].members[i] !=
        address(0)
      ) {
        if (check == index) {
        ...
        i ==
        _permissionsEnumerableStorage().roleMembers[role].indexOf[address(0)]
      ) {
        check += 1;
      }
    }
  ...
  function _removeMember(bytes32 role, address account) internal {
    uint256 idx = _permissionsEnumerableStorage().roleMembers[role].indexOf[
      account
    ];
