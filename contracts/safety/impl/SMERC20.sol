 ...
 */
abstract contract SM1ERC20 is IERC20, StakedBalances, SMPowerDelegation {
  ...
  function balanceOf(
    address account
  ) public view override(SMPowerDelegation, IERC20) returns (uint256) {
    return
  ...
  function transferFrom(
    ...
    _approve(
      sender,
      msg.sender,
      _ALLOWANCES_[sender][msg.sender].sub(
        amount,
        "SM1ERC20: transfer amount exceeds allowance"
      )
    );
    return true;
  }
  ...
  function increaseAllowance(
    ...
    _approve(
      msg.sender,
      spender,
      _ALLOWANCES_[msg.sender][spender].add(addedValue)
    );
    return true;
  }
