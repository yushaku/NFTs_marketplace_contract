 ...
 * exchange rate, may not correspond one-to-one with the underlying token. See Staking.sol.
 *
 ...
 *
 *   User rewards must be settled via Rewards any time a user's active balance changes. Special
 ...
 */
abstract contract StakedBalances is Rewards {
  ...
  constructor(
    ...
    Rewards(rewardsToken, rewardsTreasury, distributionStart, distributionEnd)
  {}
  ...
  function getActiveBalanceCurrentEpoch(
    ...
    (SMTypes.StoredBalance memory balance, , , ) = _loadActiveBalance(
  ...
  function getInactiveBalanceCurrentEpoch(
    ...
    SMTypes.StoredBalance memory balance = _loadInactiveBalance(
   ...
   */
  function getTotalInactiveBalanceCurrentEpoch() public view returns (uint256) {
    if (!hasEpochZeroStarted()) {
      return 0;
    }
    SMTypes.StoredBalance memory balance = _loadInactiveBalance(
  ...
  function _settleBalance(
    ...
    SMTypes.StoredBalance storage balancePtr = _getBalancePtr(
      maybeStaker,
      isActiveBalance
    );
    SMTypes.StoredBalance memory balance = _loadBalanceForUpdate(
      balancePtr,
      maybeStaker,
      isActiveBalance
    );
  ...
  function _increaseCurrentAndNextBalances(
    ...
    balance.currentEpochBalance = originalCurrentBalance
      .add(amount)
      .toUint240();
    balance.nextEpochBalance = uint256(balance.nextEpochBalance)
      .add(amount)
      .toUint240();
  ...
  function _getBalancePtr(
    address maybeStaker,
    bool isActiveBalance
  ) private view returns (SMTypes.StoredBalance storage) {
    // Active.
    if (isActiveBalance) {
  ...
  function _loadBalanceForUpdate(
    SMTypes.StoredBalance storage balancePtr,
    address maybeStaker,
    bool isActiveBalance
  ) private returns (SMTypes.StoredBalance memory) {
    // Active balance.
    if (isActiveBalance) {
      (
        SMTypes.StoredBalance memory balance,
  ...
  function _loadActiveBalance(
    ...
    private
    view
    returns (SMTypes.StoredBalance memory, uint256, uint256, bool)
  {
    SMTypes.StoredBalance memory balance = balancePtr;
  ...
  function _loadInactiveBalance(
    SMTypes.StoredBalance storage balancePtr
  ) private view returns (SMTypes.StoredBalance memory) {
    SMTypes.StoredBalance memory balance = balancePtr;
