 ...
 * Note that the snapshot stored at a given block number represents the value as of the end of that block.
 */

abstract contract Snapshots {
  /**
  ...
  function _writeSnapshot(
    mapping(uint256 => SMTypes.Snapshot) storage snapshots,
    ...
    if (
      ...
      snapshots[snapshotCount] = SMTypes.Snapshot(currentBlock, newValue);
  ...
  function _findValueAtBlock(
    ...
    require(blockNumber <= block.number, "Snapshots: INVALID_BLOCK_NUMBER");
    ...
    while (upper > lower) {
      ...
      SMTypes.Snapshot memory snapshot = snapshots[center];
