...
import "../../interfaces/IPowerDelegation.sol";
import "./ExchangeRate.sol";
import "./SMTypes.sol";
import "./Storage.sol";

/**
 * @title SMPowerDelegation
 *
 *
 ...
 */
abstract contract SMPowerDelegation is SM1ExchangeRate, IPowerDelegation {
  ...
  function delegateByTypeBySig(
    ...
    require(signer != address(0), "SMPowerDelegation: INVALID_SIGNATURE");
    require(nonce == _NONCES_[signer]++, "SMPowerDelegation: INVALID_NONCE");
    require(block.timestamp <= expiry, "SMPowerDelegation: INVALID_EXPIRATION");
  ...
  function getPowerAtBlock(
    ...
    (
      mapping(address => mapping(uint256 => SMTypes.Snapshot))
        storage snapshots,
  ...
  function _delegateByType(
    ...
    require(delegatee != address(0), "SMPowerDelegation: INVALID_DELEGATEE");
  ...
  function _moveDelegatesByType(
    ...
    if (from != address(0)) {
      mapping(uint256 => SMTypes.Snapshot) storage fromSnapshots = snapshots[
        from
      ];
    ...
    if (to != address(0)) {
      mapping(uint256 => SMTypes.Snapshot) storage toSnapshots = snapshots[to];
  ...
  function _getDelegationDataByType(
    ...
    view
    returns (
      mapping(address => mapping(uint256 => SMTypes.Snapshot)) storage,
      mapping(address => uint256) storage,
      mapping(address => address) storage
    )
  {
    if (delegationType == DelegationType.VOTING_POWER) {
      return (_VOTING_SNAPSHOTS_, _VOTING_SNAPSHOT_COUNTS_, _VOTING_DELEGATES_);
    } else {
      return (
