 ...
 *  After each second, an account earns rewards `r` according to the following formula:
 *        r = R * s / S
 *
 *  Where:
 *    - `R` is the rewards distributed globally each second, also called the “emission rate.”
 *    - `s` is the account's staked balance in that second (technically, it is measured at the end of the second)
 *    - `S` is the sum total of all staked balances in that second (again, measured at the end of the second)
 *
 *  The parameter `R` can be configured by the contract owner.
 *  For every second that elapses, exactly `R` tokens will accrue to users,
 *  save for rounding errors, and with the exception that while the total staked balance is zero, no tokens will accrue to anyone.
 *
 ...
 *
 *      Rewards = _USER_REWARDS_BALANCES_[user] + userStaked * (settledGlobalIndex - _USER_INDEXES_[user]) / INDEX_BASE
 *
 **/

abstract contract Rewards is EpochSchedule {
   ...
   */
  function __Rewards_init() internal {
  ...
  function _settleGlobalIndexUpToTimestamp(
    ...
    uint256 indexDelta = timeDelta.mul(emissionPerSecond).mul(INDEX_BASE).div(
      totalStaked
    );
