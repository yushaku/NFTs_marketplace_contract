...
describe("Kuiper Marketplace", () => {
  ...
  before(async () => {
    ...
    expect(await factory.getAddress()).not.eq(
      null,
      "Deploy factory is failed.",
    );
    ...
    expect(await marketplace.getAddress()).not.eq(
      null,
      "Deploy marketplace is failed.",
    );

    const Token = new USDT__factory(owner);
    payableToken = await Token.deploy();
    const uAddress = await payableToken.getAddress();
    ...
    const tx = await factory
      .connect(creator)
      .create("Kuiper Collection", "KUIPER", 10);
